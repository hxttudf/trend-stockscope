...
def _d3_list(conn, date):
    ...
    return conn.execute(
        ...
        "WHERE ts=(SELECT MAX(ts) FROM preview_signals) AND signal_date=? AND d3=1 ORDER BY symbol", (date,)).fetchall()
...
def _w30_list(conn, date):
    ...
    return conn.execute(
        "SELECT symbol, name, signal_type, signal_date, price, ref_zd, ref_zg, status, strength, strength_score FROM chanlun_signals "
        "WHERE ts=(SELECT MAX(ts) FROM preview_signals) AND signal_date=? AND w30=1 ORDER BY symbol", (date,)).fetchall()
    ...
    try:
        if preview:
            ...
            pv_dates = [r[0] for r in conn.execute(
                "SELECT DISTINCT signal_date FROM preview_signals WHERE ts=(SELECT MAX(ts) FROM preview_signals) AND status='preview'").fetchall()]
            if typ in ("二三买", "二三卖"):
                ...
                pv_rows = conn.execute(
                    ...
                    "WHERE a.ts=(SELECT MAX(ts) FROM preview_signals) AND a.status='preview' AND a.signal_type=? AND b.signal_type=? "
                    "GROUP BY a.signal_date", (bt, st)).fetchall()
                merged = {}
            ...
            if pv_dates:
                ...
                pv_map = {}
                for r in conn.execute(
                        "SELECT signal_date, signal_type, COUNT(*) FROM preview_signals WHERE ts=(SELECT MAX(ts) FROM preview_signals) AND status='preview' "
            ...
            if date in pv_dates:
                if typ == "d3":
                    ...
                    rows = conn.execute(
                        ...
                        "WHERE a.ts=(SELECT MAX(ts) FROM preview_signals) AND a.signal_date=? AND ((a.signal_type='二买' AND b.signal_type='三买') "
                    ...
                    rows = conn.execute(
                        ...
                        "FROM preview_signals a JOIN preview_signals b "
                        "ON a.symbol=b.symbol AND a.signal_date=b.signal_date "
                        "WHERE a.ts=(SELECT MAX(ts) FROM preview_signals) AND a.signal_date=? AND ((a.signal_type='二卖' AND b.signal_type='三卖') "
                        "OR (a.signal_type='三卖' AND b.signal_type='二卖')) "
                        "ORDER BY a.symbol, a.signal_type", (date,)).fetchall()
                else:
                    q = "SELECT symbol, name, signal_type, signal_date, price, ref_zd, ref_zg, status, strength, strength_score FROM preview_signals WHERE ts=(SELECT MAX(ts) FROM preview_signals) AND signal_date=?"
                    args = [date]
                    if typ:
                        q += " AND signal_type=?"
                        args.append(typ)
